...
- Press `S` from the **file areas list** or from within a **per-area file browser** to **search files across every area you can access**. Enter a search term (minimum 2 characters); filenames and short descriptions are matched, and results from all accessible areas are shown in one flat list with the area tag prepended (`[AREA] filename`). Selecting a result opens the same file info modal (and download action) as the normal browser. A blank entry or Esc returns to the list without disturbing your place. Private areas, guest restrictions and the approved-file filter apply exactly as they do on the web.
