...
| `hide_from_web` | boolean | `false` | When `true`, hides this door from the web Experience inventory and blocks its `/games/nativedoors/{doorid}` web player page (`404`). The door remains launchable over telnet/SSH via **[Files] → Door Games** (`telnet/src/DoorHandler.php`). Use this for doors that only make sense in a terminal or that a sysop wants restricted to the terminal server. |
