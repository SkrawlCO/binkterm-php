...
- [Doot Games backend (Crossroads)](Crossroads/doot-backend/README.md) — Curated Experience: the full canonical Doot Games party-game platform (all 38 first-party games) self-hosted behind a thin WebDoor redirect, with a self-hosted CLASP relay (`lumencanvas/clasp` pin) replacing the public `relay.clasp.to` for all live-room traffic; pins, build recipes, same-origin `/doot-relay` + `/doot-app` routing, and the AI/object-storage disclosure
