...
| Presentation theme | `NavigationTheme` / `NavigationThemeLoader` / `NavigationThemeConfig` / `TemplateArtSanitizer` / `ThemedNavigationRenderer` / `AnsiScreenBuffer` / `NavigationRendererFactory` | Optional. `config/terminal_theme.json` (schema 1: `MENU` + `FOOTER`; schema 2: adds `STATUS` + `DESCRIPTION`, optional `root_only`; both exact `80x24`) points at a trusted `telnet/screens/<token>.ans`. `TemplateArtSanitizer` strips everything but SGR + printable text and converts charset (CP437/SAUCE aware). `ThemedNavigationRenderer` decorates `NavigationScreenRenderer` — paints the template, positions `composeRegions()` (M1) or `composeSemanticRegions()` (M2) output into the rectangles — and **falls back to it per frame** for any non-themed geometry, ASCII/mono terminal, missing/unsafe template, or validation failure. ANSI is presentation only: no structure, actions, ACS, or scripting. `AnsiScreenBuffer` flattens the positioned output to a grid for the F6 browser preview. |
| M2 composition | `NavigationScreenRenderer::composeSemanticRegions()` | Uses resolved model data only: compact MENU, cursor-selected DESCRIPTION, STATUS from ambient text and badge annotations, and FOOTER key hints. MENU/FOOTER overflow rejects the themed frame. All rectangles are cleared and fitted; schema-1 behavior is preserved. One root front-door example is supplied; see [M2 details and activation](TerminalNavigationFramework.md#m2-first-slice-semantic-composition-at-80x24). |
...
| Write boundary | `NavigationConfigWriter` / `NavigationWriteResult` + admin-daemon `save_terminal_navigation_config` | Validated, path-constrained, atomic (temp + fsync + rename) write of a definition. Used by the existing F6 admin editor; theme configuration remains read-only in that editor. See [TerminalNavigationFramework.md](TerminalNavigationFramework.md#saving-a-definition-backend). |
