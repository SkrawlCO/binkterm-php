import { getErrors } from "./sudoku.js";
import { findTechniqueHint } from "./technique-hint.js";
function peersOf(row, col) {
    const peers = [];
    const seen = new Set();
    const add = (r, c) => {
        if (r === row && c === col)
...
function candidatesAt(board, row, col) {
    const used = new Set();
...
function eliminatingCells(board, row, col) {
    return peersOf(row, col).filter(({ row: r, col: c }) => board[r][c].value !== null);
}
...
function nakedSingleAt(board, row, col) {
...
function findEliminatorsForDigit(board, row, col, digit, excludeGroupType, excludeGroupIndex) {
    const eliminators = [];
...
function cellsOfGroup(type, index) {
    const cells = [];
    if (type === "row") {
        for (let c = 0; c < 9; c++)
...
function findHiddenSingleInGroup(board, type, index) {
    ...
    for (let d = 1; d <= 9; d++) {
        ...
        if (possibleCells.length === 1) {
            ...
            const related = groupCells.filter(({ row, col }) => (row !== cell.row || col !== cell.col) &&
                board[row][col].value !== null);
            for (const other of emptyCells) {
                if (other === cell)
                    continue;
                if (!other.candidates.has(d)) {
                    const eliminators = findEliminatorsForDigit(board, other.row, other.col, d, type, index);
 ...
 */
export function findHint(board, solution, selectedCell) {
