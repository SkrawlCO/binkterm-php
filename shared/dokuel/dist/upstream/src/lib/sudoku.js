...
import { digPuzzle, generateSolvedGrid, solve } from "./solver.js";
...
function randomClueTarget(band, rng) {
...
const GRADED_DIGS = {
    ...
    hard: {
        ...
        fallbackScore: (grade) => (grade.stuckCells === 0 ? 1000 : 0) + grade.tier * 100 - grade.stuckCells,
    },
    expert: {
        ...
        attempts: GRADE_ATTEMPTS,
        accepts: (grade) => grade.tier === 5 && grade.stuckCells >= EXPERT_MIN_STUCK,
        // The higher tier, then the deeper stuck.
        fallbackScore: (grade) => grade.tier * 100 + grade.stuckCells,
    },
};
function generateGradedPuzzle(spec, rng) {
    let best = null;
 ...
 */
export function generatePuzzleWithSolution(difficulty, rng = Math.random) {
    if (difficulty !== "easy") {
...
export function generatePuzzle(difficulty, rng = Math.random) {
 ...
 */
export function isBoardComplete(board, conflicts) {
