 ...
 */
function search(grid, masks, cap, found, out) {
    let best = -1;
    let bestMask = 0;
    let bestCount = 10;
    for (let i = 0; i < 81; i++) {
...
function countSolutionsGrid(grid, cap) {
...
function fillGrid(grid, masks, cell, rng) {
    if (cell === 81)
        return true;
    const r = Math.floor(cell / 9);
    const c = cell % 9;
    const b = boxIndex(cell);
    const avail = ALL_DIGITS & ~(masks.rows[r] | masks.cols[c] | masks.boxes[b]);
    if (avail === 0)
        return false;
    const digits = [];
    for (let v = 1; v <= 9; v++) {
    ...
    for (const v of digits) {
        ...
        masks.cols[c] |= bit;
        masks.boxes[b] |= bit;
        if (fillGrid(grid, masks, cell + 1, rng))
            return true;
        grid[cell] = 0;
        masks.rows[r] &= ~bit;
        masks.cols[c] &= ~bit;
 ...
 */
export function digPuzzle(solved, targetClues, rng = Math.random) {
    const grid = toGrid(solved);
    if (!grid)
        throw new Error("digPuzzle requires a valid 81-char grid");
