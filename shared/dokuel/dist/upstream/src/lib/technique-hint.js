 ...
 */
import { boxIndex } from "./board-geometry.js";
import { findUnlockingPlacement, } from "./techniques.js";
function toPosition(cell) {
...
function sharedLineName(cells) {
    const rows = new Set(cells.map((c) => Math.floor(c / 9)));
    if (rows.size === 1)
        return `row ${[...rows][0] + 1}`;
    return `column ${(cells[0] % 9) + 1}`;
}
function listDigits(digits, joiner) {
    if (digits.length === 1)
        return String(digits[0]);
    return `${digits.slice(0, -1).join(", ")} ${joiner} ${digits.at(-1)}`;
}
function describeElimination(e) {
...
function buildTechniqueHint(unlock) {
    ...
    return {
        ...
        explanation: preamble + describeElimination(elimination) + describeConsequence(unlock),
