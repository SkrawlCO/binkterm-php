...
const root='/root/binktermphp/app/shared/dokuel/web/dist';
http.createServer(async(req,res)=>{
 if(req.url==='/state'){
  let body='';for await(const b of req){body+=b;if(body.length>110000){res.writeHead(413).end();return;}}
  const child=spawn('docker',['exec','-i','binkterm-app','php','/var/www/html/shared/dokuel/persistence/review/proxy.php']);let output='';child.stdout.on('data',b=>output+=b);child.stderr.resume();
...
}).listen(43203,'127.0.0.1',()=>console.log('Loopback review server 43203'));
