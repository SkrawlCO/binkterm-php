...
const target=new URL('../../public_html/webdoors/wordwright/assets/',import.meta.url);
...
for(const name of await readdir(assets))if(/^(index|lists-[456])-[\w-]+\.(js|css)$/.test(name))await unlink(new URL(name,assets));
