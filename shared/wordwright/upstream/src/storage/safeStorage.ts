...
function createBrowserStorage(storage: Storage): SafeStorage {
  return {
    getItem: (key) => {
      try {
 ...
 */
export function createSafeStorage(backend: Storage | undefined): SafeStorage {
  if (!backend || !probe(backend)) return createMemoryStorage();
  return createBrowserStorage(backend);
}

function resolveBackend(): Storage | undefined {
  try {
