...
SimpleRouter::group(['prefix' => '/api'], function() {
    ...
    SimpleRouter::get('/dashboard/stats', function() {
        ...
        // Terminal main-menu Crossroads signal — the same reducer the terminal
        // daemons call in-process. Shared so both consumers stay identical.
        // Any failure inside compose() is contained and yields null.
        $stats['crossroads'] = \BinktermPHP\Crossroads\TerminalDashboardSignal::compose($user);
    ...
    SimpleRouter::get('/echoareas', function() {
        ...
        // Shared user-scoped listing (also called in-process by the terminal/SSH
        // daemons — see EchoareaManager::listForUser()).
        $echoareas = (new \BinktermPHP\EchoareaManager())->listForUser($user, [
            'filter' => $_GET['filter'] ?? 'active',
            'subscribed_only' => (($_GET['subscribed_only'] ?? 'false') === 'true'),
        ]);
